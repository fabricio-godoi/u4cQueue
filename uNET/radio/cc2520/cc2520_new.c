...
//	int s = splhigh();
	...
	// Configure interruption event (falling edge)
	CC2520_DISABLE_SFD_INT();
	CC2520_SFD_INT_INIT();
...
#define LOOP_20_SYMBOLS CC2520_CONF_SYMBOL_LOOP_COUNT
	...
	for (i = LOOP_20_SYMBOLS; i > 0; i--) {
		...
		if (CC2520_SFD_IS_1) {
			 ...
			 */
			is_cc2520_txing = TRUE;
			UNET_RADIO.set(TX_STATUS, RADIO_TX_WAIT);

			// Wait end of transmission, signed in the SFD interruption
	...
	// Return the radio to RX
	NODESTAT_UPDATE(radiocol);
	CC2520_PRINTF("cc2520: TX Collision, transmission never start\n");
	UNET_RADIO.set(TX_STATUS, RADIO_TX_ERR_COLLISION); // RADIO_TX_WAIT
	UNET_RADIO.set(TX_RETRIES, 0);
	return RADIO_TX_ERR_COLLISION;
}
...
int cc2520_write(const void *buf, uint16_t len) {
	...
	// Put the packet in the radio
	cc2520_prepare(buf, len);
	ret = cc2520_transmit();
	return ret;
}
/*---------------------------------------------------------------------------*/
 ...
 */
volatile int ifcall[2];
int32_t cc2520_get_rxfifo(uint8_t *buf, uint16_t buf_len) {
	volatile uint8_t len;
	uint8_t footer[2];

	if (!CC2520_FIFOP_IS_1) {
		// Something is wrong, probably some packet is beeing received
		return 0;
	}
...
/**
 * SFD Interrupt:
 * 1 - In TX indicates TX is done sending packet
 * 2 - In RX indicates that a complete frame has been received
 */
//static char ack_packet[CC2520_ACK_LENGTH];
extern INT8U iNesting;
#define interrupt(x) void __attribute__((interrupt (x)))
interrupt(PORT2_VECTOR) Radio_TX_Interrupt(void) {
	// ************************
	// Interruption entry
	// ************************
	OS_INT_ENTER();
	...
	if (P2IFG & BV(CC2520_SFD_PIN)) {
		...
		if(is_cc2520_txing){
			...
			is_cc2520_txing = FALSE;

			// Will need to wait ACK?
			if(!is_radio_tx_ack(cc2520_radio_params.param[RADIO_STATE])){

				// Broadcast message, no ack needed
				UNET_RADIO.set(TX_STATUS, RADIO_TX_ERR_NONE);
				UNET_RADIO.set(TX_RETRIES, 0);

				// ACK is not needed, so release the unet core
				RADIO_STATE_SET(TX_OK);
			}

		}
//		/// RECEIVING, see Radio_RX_Interrupt
//		else{
//			cc2520_rx_disable(); // Disable to not overflow or flush rx buffer
//			// Check if it's a ACK message
//			final_status = getreg(CC2520_RXFIFOCNT);
//			if(final_status == CC2520_ACK_LENGTH){
...
//					UNET_RADIO.set(TX_RETRIES, 0);
//
//					// ACK was expected, so release the unet core
//					RADIO_STATE_SET(TX_OK);
//				}
//
//				flushrx();
//				cc2520_rx_enable(); // Re-enable RX
...
//#define interrupt(x) void __attribute__((interrupt (x)))
/** NOTE:
 *       Even this is the RX interruption manager,
 *       in the TX_Interrupt occur when the frame is read,
 *       BUT this interruption occur first!
 */
volatile char debug_one=0, debug_two=0;
volatile char rxfifocnt;
...
//		strobe(CC2520_INS_SRFOFF); // this shutdown the radio
		cc2520_rx_disable(); // Disable to not overflow or flush rx buffer

		debug_one = CC2520_FIFOP_IS_1;
		rxfifocnt = getreg(CC2520_RXFIFOCNT);
		debug_two = CC2520_FIFOP_IS_1;
		// Check if it's a ACK message
		if(rxfifocnt == CC2520_ACK_LENGTH){
//				CC2520_READ_FIFO_BUF(ack_packet, CC2520_ACK_LENGTH);
...
//			strobe(CC2520_INS_SRXON);
		...
		// Packets smaller then ACK is a bug
		else if(rxfifocnt > CC2520_ACK_LENGTH){
			RADIO_STATE_SET(RX_OK);
		}


//		if(!CC2520_FIFOP_IS_1) printf("FOP\n");
...
//		if(is_cc2520_txing){
//
//			// Radio finished the transmission
//			is_cc2520_txing = FALSE;
//
//			// Put radio to RX
//			cc2520_rx_enable();
...
//				UNET_RADIO.set(TX_STATUS, RADIO_TX_WAIT);
//				CC2520_PRINTF("cc2520: TX WAIT ACK\n"); // waiting ack
//			}
//			else {
//				// Broadcast message, no ack needed
//	//				radio_tx_acked(TRUE);
