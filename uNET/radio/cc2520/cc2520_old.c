	...
	for (i = LOOP_20_SYMBOLS; i > 0; i--) {
		...
		if (CC2520_SFD_IS_1) {
			...
			if (!(status() & BV(CC2520_TX_ACTIVE))) {
				...
				CC2520_PRINTF("cc2520: TX Collision on send\n");
...
int cc2520_write(const void *buf, uint16_t len) {
	...
	// If we get here, the controller isn't txing,
	// so flush the data on the radio tx buffer to
	// ensure that has no garbage there.
//	flushtx();
//	if((status() & BV(CC2520_TX_ACTIVE))){
//		cc2520_rx_enable(); // abort TX
//		printf("cc2520: tx still running\n");
////		return -1;
//	}
...
//	if((ret = cc2520_transmit()) == RADIO_TX_ERR_COLLISION){
//		NODESTAT_UPDATE(radiocol);
//		UNET_RADIO.set(TX_STATUS, RADIO_TX_ERR_COLLISION); // RADIO_TX_WAIT
//		UNET_RADIO.set(TX_RETRIES, 0);
//		radio_txing(TRUE); // set this to true, to isr handler be able to release the lock
...
int32_t cc2520_get_rxfifo(uint8_t *buf, uint16_t buf_len) {
	...
	// This will never happen in this OS, since the only
	// way to reach here is by FIFOP interruption
	if (!CC2520_FIFOP_IS_1) {
		printf("cc2520: fifop err\n");
		return 0;
	}
