...
uint8_t ieee802154_packet_input(packet_t *p)
{
...
#if RADIO_AUTOCRC == TRUE
	UNET_RADIO.get(CRC, &crc); // TODO this information shouldn't be get from radio, instead use some variable in packet_t
	if(crc == FALSE){
...
#endif
	{
		...
		if(link_packet_is_duplicated(&(p->packet[MAC_SRC_16]), p->packet[MAC_SEQ_NUM]) == TRUE)
		{
			...
			// TODO check if this feature here will cause the packets to be lost,
			//      if it's OK, so enable it to prevent further processing
